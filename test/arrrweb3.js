...
describe('ARRRweb3', () => {
  const PIRATE_ADDRESS = getDefaultPirateAddress();
  let arrrweb3;

  beforeEach(() => {
    arrrweb3 = new ARRRweb3(getPirateRPCAddress());
  });
  ...
  describe('getBlockchainInfo()', () => {
    it('returns the block info', async () => {
      const res = await arrrweb3.getBlockchainInfo();
      assert.isDefined(res);
      assert.isObject(res);
  ...
  describe('getBlockCount()', () => {
    it('returns the blockcount', async () => {
      const res = await arrrweb3.getBlockCount();
      assert.isDefined(res);
      assert.isNumber(res);
    });
  });
  ...
  describe('getBlockHash()', () => {
    it('returns the block hash', async () => {
      const res = await arrrweb3.getBlockHash(0);
      assert.isDefined(res);
      assert.isString(res);
    });
  });
  ...
  describe('getPeerInfo()', () => {
    it('returns the Node info', async () => {
      const res = await arrrweb3.getPeerInfo();
      assert.isDefined(res);
      assert.isArray(res);
  ...
  describe('validateAddress()', () => {
    it('returns an object validating the address', async () => {
      let res = await arrrweb3.validateAddress(PIRATE_ADDRESS);
      assert.isDefined(res);
      assert.isDefined(res.isvalid);

      res = await arrrweb3.validateAddress('helloworld');
      assert.isDefined(res);
      assert.isDefined(res.isvalid);
    });
  });
  ...
  describe('dumpPrivateKey()', () => {
    it('returns the private key', async () => {
      const address = await arrrweb3.getAccountAddress('');
      if (await isWalletEncrypted(arrrweb3)) {
        await arrrweb3.walletPassphrase(getWalletPassphrase(), 3600);
        assert.isTrue((await arrrweb3.getWalletInfo()).unlocked_until > 0);

        const res = await arrrweb3.dumpPrivateKey(address);
        assert.isDefined(res);
        assert.isString(res);

        await arrrweb3.walletLock();
        assert.isTrue((await arrrweb3.getWalletInfo()).unlocked_until === 0);
      } else {
        const res = await arrrweb3.dumpPrivateKey(address);
        assert.isDefined(res);
  ...
  describe('zExportKey()', () => {
    it('returns the z_private key', async () => {
      const address = await arrrweb3.getNewAddress();
      if (await isWalletEncrypted(arrrweb3)) {
        await arrrweb3.walletPassphrase(getWalletPassphrase(), 3600);
        assert.isTrue((await arrrweb3.getWalletInfo()).unlocked_until > 0);
        const res = await arrrweb3.zExportKey(address);
        assert.isDefined(res);
        assert.isString(res);
        await arrrweb3.walletLock();
        assert.isTrue((await arrrweb3.getWalletInfo()).unlocked_until === 0);
  ...
  describe('getUnconfirmedBalance()', () => {
    it('returns the unconfirmed balance', async () => {
      const res = await arrrweb3.getUnconfirmedBalance();
      assert.isDefined(res);
      assert.isNumber(res);
    });
  });
  ...
  describe('listAddressGroupings()', () => {
    it('returns an array of address groupings', async () => {
      const res = await arrrweb3.listAddressGroupings();
      assert.isDefined(res);
      assert.isArray(res);
  ...
  describe('listLockUnspent()', () => {
    it('returns an array of unspendable outputs', async () => {
      const res = await arrrweb3.listLockUnspent();
      assert.isDefined(res);
      assert.isArray(res);
    });
  });
  ...
  describe('listUnspent()', () => {
    it('returns an unspent output array', async () => {
      const res = await arrrweb3.listUnspent();
      assert.isDefined(res);
      assert.isArray(res);
    });
  });
  ...
  describe('walletLock()', () => {
    it('locks the encrypted wallet', async () => {
      if (await isWalletEncrypted(arrrweb3)) {
        await arrrweb3.walletPassphrase(getWalletPassphrase(), 3600, true);
        assert.isTrue((await arrrweb3.getWalletInfo()).unlocked_until > 0);
  ...
  !_.includes(process.argv, '--cleanenv') ? describe.skip : describe('cleanEnv tests', () => {
    describe('getNewAddress()', () => {
      it('returns a new runebase address', async () => {
        const res = await arrrweb3.getNewAddress('');
        assert.isDefined(res);
        assert.isString(res);
        assert.isTrue(res.startsWith('5') || res.startsWith('R'));
      });
    ...
    describe('backupWallet()', () => {
      it('backup the wallet', async () => {
        const res = await arrrweb3.backupWallet(path.join(__dirname, './data/backup.dat'));
    ...
    describe('importWallet()', () => {
      it('throw an error if importing a non-existent file', async () => {
        try {
          await arrrweb3.importWallet(path.join(__dirname, './data/backup.dat'));
        } catch (err) {
      ...
      it('import the wallet from a wallet dump file', async () => {
        const res = await arrrweb3.importWallet(path.join(__dirname, './data/backup.dat'));
        assert.notTypeOf(res, 'Error');
      });
    });
  });
